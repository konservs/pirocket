...
double Altimeter::getAltitude() {
	...
	altitude = getAltitude(pressure, (((double)temperature) / 10) + 273.15);
	return altitude;
	}

double Altimeter::getAltitude(double i, double k) {
	double M = 0.0289644; //\mu — молярная масса воздуха, 29 г/моль (в расчете используется 0.029 кг/моль)
	double g = 9.80665; //g — ускорение силы тяжести, 9.81 м/(сс)
	double R = 8.31432; //R — универсальная газовая постоянная, 8.31 Дж/(мольК)
	double a = 101325; //Pressure at sea level

	if ((a / i) < ((double)101325 / (double)22632.1))
	{
		double d = (double)-0.0065;
		double e = (double)0;
		double j = pow((i / a), (R * d) / (g * M));
		return e + ((k * ((1 / j) - 1)) / d);
	}
	else
	{
		if ((a / i) < ((double)101325 / (double)5474.89))
		{
			double e = 11000;
			double b = k - 71.5;
			double f = (R * b * (log(i / a))) / ((-g) * M);
			double l = 101325;
			double c = 22632.1;
			double h = ((R * b * (log(l / c))) / ((-g) * M)) + e;
			return h + f;
		}
	}
	return 0;
...
void Altimeter::bmp085_Calibration() {
...
// Returns a file id
int Altimeter::bmp085_i2c_Begin() {
	int fd;
...
__s32 Altimeter::bmp085_i2c_Read_Int(int fd, __u8 address) {
...
void Altimeter::bmp085_i2c_Write_Byte(int fd, __u8 address, __u8 value) {
...
void Altimeter::bmp085_i2c_Read_Block(int fd, __u8 address, __u8 length, __u8 *values) {
...
unsigned int Altimeter::bmp085_ReadUT() {
	unsigned int ut = 0;
	int fd = bmp085_i2c_Begin();
...
unsigned int Altimeter::bmp085_ReadUP() {
	unsigned int up = 0;
	int fd = bmp085_i2c_Begin();
...
unsigned int Altimeter::bmp085_GetPressure(unsigned int up) {
...
unsigned int Altimeter::bmp085_GetTemperature(unsigned int ut) {
	int x1, x2;
